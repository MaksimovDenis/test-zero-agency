...
import (
	...
	"test-zero-agency/package/handler"
...
func main() {
	...
	err = os.Chmod("logs/app.log", 0666)
	if err != nil {
		panic(err)
	}
	...
	srv := new(zeroAgency.Server)
	go func() {
		if err := srv.Run(config.Port, handlers.InitRoutes()); err != nil {
			logger.Log.Fatalf("error occurred while running http server: %s", err.Error())
		}
	}()
